...
{
    public partial class PrintPrinting : Form
    {
        ...
        private string __AUTHCODE__ = CheckAuth.ValidCheck(CommonValues.packageNo, 89, 0);   // 패키지번호, 프로그램번호, 윈도우번호

        public PrintPrinting()
        {
            InitializeComponent();
        }
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    rptPrintPrinting report = new rptPrintPrinting();
        ...
        private void DataBinding_Order()
        {
            try
            {
                ...
                dsData = Dev.Controller.Printing.Getlist(_searchKey, _searchString); //, dtFromDate.Value, dtToDate.Value);
            }
            catch (Exception ex)
            {
