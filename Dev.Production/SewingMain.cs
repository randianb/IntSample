...
namespace Dev.Production
{
    ...
    /// 나염 외주오더현황 
    /// </summary>
    public partial class SewingMain : InheritForm
    {
        #region 1. 변수 설정
        ...
        private Controller.Sewing _obj1 = null;                                // 현재 생성된 객체 
        ...
        private List<CustomerName> lstFabric = new List<CustomerName>();        // 
        private List<CustomerName> lstFabric2 = new List<CustomerName>();        // 
        
        private string _layoutfile = "/GVLayoutSewing.xml";
        ...
        public SewingMain(InheritMDI main, string WorkOrderIdx)
        {
        ...
        private void GV1_CreateColumn(RadGridView gv)
        {
            ...
            GridViewTextBoxColumn OutOrderIdx = new GridViewTextBoxColumn();
            OutOrderIdx.Name = "OutOrderIdx";
            OutOrderIdx.FieldName = "OutOrderIdx";
            OutOrderIdx.IsVisible = false;
            gv.Columns.Add(OutOrderIdx);
        ...
        private void DataLoading_DDL()
        {
            ...
            foreach (DataRow row in _dt.Rows)
            {
                lstFabric.Add(new CustomerName(Convert.ToInt32(row["CustIdx"]),
                                            row["CustName"].ToString(),
                                            Convert.ToInt32(row["Classification"])));
                lstFabric2.Add(new CustomerName(Convert.ToInt32(row["CustIdx"]),
                                            row["CustName"].ToString(),
                                            Convert.ToInt32(row["Classification"])));
            }
                                    ...
                                    Dictionary<Options.CommonValues.KeyName, string> SearchString)
        {
            try
            {
                ...
                _ds1 = Controller.Sewing.Getlist(SearchKey, SearchString);
                if (_ds1 != null)
                {
        ...
        private void GV1_Update(object sender, GridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                // 객체생성 및 값 할당
                _obj1 = new Controller.Sewing(Convert.ToInt32(_gv1.Rows[row.Index].Cells["Idx"].Value));
                ...
                if (row.Cells["OutOrderIdx"].Value!=null && row.Cells["OutOrderIdx"].Value != DBNull.Value)
                    _obj1.OutOrderIdx = row.Cells["OutOrderIdx"].Value.ToString(); else _obj1.OutOrderIdx = "";
                if (row.Cells["WorkQty"].Value != DBNull.Value) _obj1.WorkQty = Convert.ToInt32(row.Cells["WorkQty"].Value.ToString()); else _obj1.WorkQty = 0;
                if (row.Cells["Worked"].Value != DBNull.Value) _obj1.Worked = Convert.ToInt32(row.Cells["Worked"].Value.ToString()); else _obj1.Worked = 0;
                if (row.Cells["WorkType"].Value !=null && row.Cells["WorkType"].Value != DBNull.Value) _obj1.WorkType = Convert.ToInt32(row.Cells["WorkType"].Value.ToString()); else _obj1.WorkType = 0;
                if (row.Cells["Remarks"].Value != DBNull.Value) _obj1.Remarks = row.Cells["Remarks"].Value.ToString(); else _obj1.Remarks = "";

                if (row.Cells["WorkDate"].Value != DBNull.Value && row.Cells["WorkDate"] != null)
                {
                    _obj1.WorkDate = Convert.ToDateTime(row.Cells["WorkDate"].Value);
                }
                else
                {
                    _obj1.WorkDate = new DateTime(2000, 1, 1);
                }

                // 업데이트 (오더캔슬, 선적완료 상태가 아닐경우)
                _bRtn = _obj1.Update();
                __main__.lblRows.Text = "Updated Sewing Info";
