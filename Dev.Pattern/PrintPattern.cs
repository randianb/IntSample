...
{
    public partial class PrintPattern : Form
    {
        ...
        private string __AUTHCODE__ = CheckAuth.ValidCheck(CommonValues.packageNo, 38, 0);   // 패키지번호, 프로그램번호, 윈도우번호
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    _searchKey.Add(CommonValues.KeyName.Codes, DateKind.SelectedIndex);
                    _searchKey.Add(CommonValues.KeyName.Size, Convert.ToInt32(ddlSize.SelectedValue));
                    
                    DataBinding_Order();

                    rptPrintPattern report = new rptPrintPattern();
        ...
        private void DataBinding_Order()
        {
            try
            {
                ...
                dsData = Dev.Controller.Pattern.Print(_searchString, _searchKey, dtFromDate.Value, dtToDate.Value);
            }
            catch (Exception ex)
            {
