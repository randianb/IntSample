...
namespace Dev.Codes
{
    ...
    /// </summary>
    public partial class CodeColor : InheritForm
    {
        #region 1. 변수 설정
        ...
        public CodeColor(InheritMDI main)
        {
        ...
        /// <param name="e"></param>
        private void frmCodeColor_Load(object sender, EventArgs e)
        {
        ...
        private void GV1_CreateColumn(RadGridView gv)
        {
            ...
            cColorIdx.DataType = typeof(int);
            cColorIdx.Name = "ColorIdx";
            cColorIdx.FieldName = "ColorIdx";
            cColorIdx.HeaderText = "ID";
            cColorIdx.Width = 50;
            cColorIdx.TextAlignment = ContentAlignment.MiddleLeft;
            gv.Columns.Add(cColorIdx);

            GridViewTextBoxColumn cColorName = new GridViewTextBoxColumn();
            cColorName.Name = "ColorName";
            cColorName.FieldName = "ColorName";
            cColorName.HeaderText = "ColorName";
            cColorName.Width = 300;
            cColorName.TextAlignment = ContentAlignment.MiddleLeft;
            gv.Columns.Add(cColorName);

            GridViewTextBoxColumn cClassification = new GridViewTextBoxColumn();
            cClassification.DataType = typeof(int);
            cClassification.Name = "Classification";
            cClassification.FieldName = "Classification";
            cClassification.HeaderText = "Classification";
            cClassification.IsVisible = false; 
            cClassification.Width = 20;
        ...
        private void mnuDel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (string.IsNullOrEmpty(str)) str = Convert.ToInt32(row.Cells["CodeIdx"].Value).ToString();
                    else str += "," + Convert.ToInt32(row.Cells["CodeIdx"].Value).ToString();
                }
                ...
                {
                    // 삭제후 새로고침
                    _bRtn = Controller.Color.Delete(str);
        ...
        private void mnuNew_Click(object sender, EventArgs e)
        {
            try
            {
                DataRow row = Controller.Color.Insert("", 0, 1);
        ...
        private void DataBinding_GV1(string ColorName)
        {
            try
            {
                //조회시간
                ...
                if (ColorName.Length > 0)   
                    _ds1 = Controller.Color.Getlist(ColorName);
                else
                    _ds1 = Controller.Color.Getlist();
        ...
        private void GV1_Update(object sender, GridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                // 객체생성 및 값 할당
                _obj1           = new Controller.Color(Convert.ToInt32(row.Cells["ColorIdx"].Value));
                _obj1.ColorIdx       = Convert.ToInt32(row.Cells["ColorIdx"].Value);
                _obj1.ColorName    = row.Cells["ColorName"].Value.ToString();
                _obj1.Classification   = Convert.ToInt32(row.Cells["Classification"].Value);
